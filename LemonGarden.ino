...
char *tempPtr;
int blaeser;
float rainbow, bhut, chocolate, habanero, lFront, lBack;
char habaneroPin[] = "A0";
...
char lemonFrontLine[] = "Lemon Front: ";
char lemonBackLine[] = "Lemon Back: ";
char *lFrontPtr = lemonFrontLine;
char *lBackPtr = lemonBackLine;
...
void initDHT22Screen(){
  ...
  letterByLetter(lFrontPtr,2);
  letterByLetter(lBackPtr,3);
  char percent[] = "%";
  // make it into a ptr to print it
  char *percentPTR = percent;
  for(int i=1; i<4; i++){
  ...
  char celsius[] = "C";
  char *celsiusPtr = celsius;
  lcd.setCursor(18,0);
  lcd.print((char)223); //prints degree sign
  letterByLetter(celsiusPtr, 19, 0); // Ending with a C
}
...
void updateSoilHumidity(){
  //read from analog 0 and 1
  lFront = analogRead(A0);
  lBack = analogRead(A1);
  //convert to percentages
  float lFrontPercent = (float)(lFront/maxSoilValue)*100;
  float lBackPercent = (float)(lBack/maxSoilValue)*100;
  
  //create chars to store the values in readable format to the LCD screen 
  char lFrontArray[chiliArraySize], lBackArray[chiliArraySize];

  //and then move from float to char[]
  dtostrf(lFrontPercent, 5, 1, lFrontArray);
  dtostrf(lBackPercent, 5, 1, lBackArray);

  //create pointers in order to print them
  char *lFrontPtr = lFrontArray;
  char *lBackPtr = lBackArray;

  //place the text  //TODO make a metod to place text without delay
  letterByLetter(lFrontPtr, 12, 2);
  letterByLetter(lBackPtr, 12, 3);
}
